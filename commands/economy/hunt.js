...
module.exports = {
  ...
  async execute (bot, message, args) {
    ...
    const small = [
      `Spider`, `Snail`, `Earthworm`, `Mosquito`, `Fly`,`Dragonfly`,`Cockroach`,`Ant`,`Firefly`,`Butterfly`,`Bee`
    ]

    const medium = [
      `Lemming`,`Frog`,`Chamelon`,`Hamster`,`Squirrel`,`Weasel`, `Rabbit`,`Chicken`,`Duck`
      ]

    const big = [
      `Lion`, `Tiger`, `Cheetah`, `Hyena`, `Rhino`, `T-rex`
      ]
    //if (await Player.setCooldown('hunt')) return

    const checkCd = await Player.checkCooldown('hunt')
    if (!checkCd) await Player.setCooldown('hunt')
    else return
        ...
        if ( p < SmallRange) {
          ...
          message.channel.send({
            embed: {
              ...
              // title: ``,
              description: `${emoji} | You have hunt nothing. :(`,
        ...
        if (animal) {
          ...
          message.channel.send({
            embed: {
              ...
              author: { name: `${Player.player.tag}`, icon_url: `${Player.player.displayAvatarURL()}`},
              // title: ``,
              description: `${emoji} | You have hunt ${vowels.includes(animal[0].toLowerCase()) ? 'an' : 'a'} **${animal}** and got **${o.toLocaleString()}** ${Icons.owlet}`,
              //owlet${o>1?'s':''}!`,
